...
public class GeometryCanonicalEditPolicy extends CanonicalEditPolicy {
	...
	private boolean isMyDiagramElement(View view) {
		...
		return visualID == SimplePositionEditPart.VISUAL_ID
				|| visualID == TrackPositionEditPart.VISUAL_ID;
	}
	...
	protected void refreshSemantic() {
		...
		for (Iterator<GeometryNodeDescriptor> descriptorsIterator = childDescriptors
	...
	private Collection<IAdaptable> refreshConnections() {
		...
		Collection<GeometryLinkDescriptor> linkDescriptors = collectAllLinks(
		...
		for (Iterator linksIterator = existingLinks.iterator(); linksIterator
			...
			for (Iterator<GeometryLinkDescriptor> linkDescriptorsIterator = linkDescriptors
				...
				GeometryLinkDescriptor nextLinkDescriptor = linkDescriptorsIterator
						.next();
	...
	private Collection<GeometryLinkDescriptor> collectAllLinks(View view,
		...
		case SimplePositionEditPart.VISUAL_ID: {
			if (!domain2NotationMap.containsKey(view.getElement())) {
				result.addAll(GeometryDiagramUpdater
						.getSimplePosition_2001ContainedLinks(view));
			}
			if (!domain2NotationMap.containsKey(view.getElement())
					|| view.getEAnnotation("Shortcut") == null) { //$NON-NLS-1$
				domain2NotationMap.put(view.getElement(), view);
		...
		case TrackPositionEditPart.VISUAL_ID: {
			if (!domain2NotationMap.containsKey(view.getElement())) {
				result.addAll(GeometryDiagramUpdater
						.getTrackPosition_2002ContainedLinks(view));
			}
			if (!domain2NotationMap.containsKey(view.getElement())
					|| view.getEAnnotation("Shortcut") == null) { //$NON-NLS-1$
				domain2NotationMap.put(view.getElement(), view);
		...
		case TrackEditPart.VISUAL_ID: {
			if (!domain2NotationMap.containsKey(view.getElement())) {
				result.addAll(GeometryDiagramUpdater
						.getTrack_4001ContainedLinks(view));
			}
			if (!domain2NotationMap.containsKey(view.getElement())
					|| view.getEAnnotation("Shortcut") == null) { //$NON-NLS-1$
				domain2NotationMap.put(view.getElement(), view);
	...
	private Collection<IAdaptable> createConnections(
			Collection<GeometryLinkDescriptor> linkDescriptors,
			Map<EObject, View> domain2NotationMap) {
